...
console.log("BODY:", body);
  ...
  await db.execute({
    ...
    args: [teamId, name.trim(), payload.userId],
  });
...
.post('/teams/:teamId/join', async ({ headers, params, set }) => {
  // 1. Extract & verify token
  const token = verifyAuthToken(headers.authorization);
  ...
  // 3. Check if user exists (important safety check)
  ...
  // 4. Insert membership safely
  try {
    await db.execute({
      sql: 'INSERT INTO team_members (team_id, user_id) VALUES (?, ?)',
      args: [teamId, payload.userId],
    });
  } catch (error: any) {
    // 5. Handle duplicate membership (composite primary key violation)
    if (
